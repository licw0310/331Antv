...
export default {
      ...
      handler(val) {
        if (val) {
          if (val.isNode()) {
            //节点
            console.log(val);
            if(val.store.data.shape =='lane-y' || val.store.data.shape =='lane-x'){
              this.drawerNode.nodeText = val.store.data.attrs.text.text;
            }else{
  ...
  methods: {
    ...
    changeNodeText() {
      console.log(this.drawerNode.nodeText)
      if(this.selectCell.store.data.shape === 'lane-x' || this.selectCell.store.data.shape === 'lane-y'){
      this.selectCell.attr("text/text", this.drawerNode.nodeText);
      }else{
